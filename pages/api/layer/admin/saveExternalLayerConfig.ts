...
const saveExternalLayerConfig: NextApiHandler = async (req, res) => {
  ...
  if (data && data.layer_id) {
    try {
      if (await LayerModel.allowedToModify(data.layer_id, user_id)) {
        const result = await LayerModel.saveExternalLayerConfig(
          data.layer_id,
        ...
        if (result) {
          res.status(200).json({
            success: true,
            action: 'saveExternalLayerConfig'
          })
        } else {
          res.status(200).json({
            success: false,
...
export default saveExternalLayerConfig
