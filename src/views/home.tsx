...
export default class HomePro extends React.Component<Props, State> {
  ...
  */
  ...
  renderHomePageMap: any | ((config: any, key: string) => Node | string) = (
  ...
  renderXComponent: any | ((config: any, key: string) => Node | string) = (
    config: Record<string, any>,
    key: string
  ) => {
  ...
  renderSlides: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const style = config.style || {}
  ...
  renderOnboardingLinks: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
  ...
  renderProLinks: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const bgColor = config.bgColor ? config.bgColor : 'inherit'
  ...
  renderCarousel: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const { t } = this
    ...
    const featuredLayersCards = props.featuredLayers
      ? shuffle(props.featuredLayers.map(cardUtil.getLayerCard))
      : []
    const featuredGroupsCards = props.featuredGroups
      ? shuffle(props.featuredGroups.map(cardUtil.getGroupCard))
      : []
    const featuredMapsCards = props.featuredMaps
      ? shuffle(props.featuredMaps.map(cardUtil.getMapCard))
      : []
    ...
    const popularLayersCards = props.popularLayers
      ? shuffle(props.popularLayers.map(cardUtil.getLayerCard))
      : []
    const popularGroupsCards = props.popularGroups
      ? shuffle(props.popularGroups.map(cardUtil.getGroupCard))
      : []
    const popularMapsCards = props.popularMaps
      ? shuffle(props.popularMaps.map(cardUtil.getMapCard))
      : []
    ...
    const recentLayersCards = props.recentLayers
      ? shuffle(props.recentLayers.map(cardUtil.getLayerCard))
      : []
    const recentGroupsCards = props.recentGroups
      ? shuffle(props.recentGroups.map(cardUtil.getGroupCard))
      : []
    const recentMapsCards = props.recentMaps
      ? shuffle(props.recentMaps.map(cardUtil.getMapCard))
      : []
  ...
  renderStories: any | ((config: any, key: string) => void | Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const { t } = this
  ...
  renderText: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const style = config.style || {}
  ...
  renderHtml: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const style = config.style || {}
  ...
  renderButton: any | ((config: any, key: string) => Node) = (
    config: Record<string, any>,
    key: string
  ) => {
    const style = config.style || {}
              ...
              {this.props.pageConfig.components.map((component, i) => {
                ...
                if (!component.disabled) {
                  if (component.type === 'map') {
                    ...
                    component.type === 'onboarding-links'
                  ) {
