...
export default class CreateLayer extends React.Component<Props, State> {
  ...
  componentDidUpdate(): void {
    if (this.sourceDisplay) {
      scrollToComponent(this.sourceDisplay, {
        align: 'bottom'
      })
    }
  }
  ...
  render(): JSX.Element {
    const { t, props, state, selectSource } = this
    const { source } = state
    const { showCancel, onCancel, cancelText, mapConfig } = props
    const sourceDisplay = this.getSource(source, mapConfig, t)
    return (
      <>
        <style jsx global>
          {`
