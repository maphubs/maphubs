...
export default {
  ...
  async setOverlayStyle (overlayStyle: Object, optimizeLayers: boolean) {
    ...
    if (this.overlayMapStyle) {
      ...
      if (customSourcesToUpdate.length > 0 && customSourcesToUpdate.length === customSources.length) {
      ...
      } else if (customSourceLayers.length > 0 && layersToUpdateWithPosition.length === customSourceLayers.length) {
  ...
  removeLayer (id: string) {
    _remove(this.glStyle.layers, {id})
  },
