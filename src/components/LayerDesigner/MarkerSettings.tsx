...
export default class MarkerSettings extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    if (
      props.style.layers &&
      Array.isArray(props.style.layers) &&
      props.style.layers.length > 0
    ) {
  ...
  onFormChange: (model: any) => void = (model: Record<string, any>) => {
    if (model.size) {
    ...
    if (options.enabled) {
      style = MapStyles.markers.enableMarkers(
        this.props.style,
        options,
        this.props.layer
      )
        ...
        <Formsy ref='form' onChange={this.onFormChange}>
          ...
          >
            <Select
              ...
              value={this.state.options.shape}
              startEmpty={!this.state.options.shape}
          ...
          >
            <Select
              ...
              startEmpty={!this.state.options.size}
              tooltipPosition='right'
          ...
          >
            <Select
              ...
              label={t('Marker Icon')}
              options={iconOptions}
              value={this.state.options.icon}
              startEmpty={!this.state.options.icon}
              tooltipPosition='right'
