...
module.exports = function(app) {
   ...
   *  }]
    ...
    app.get('/relations/:id', function (req, res) {
        if(!req.params.id)
            res.send(400, {error: 'Valid relation id required.'});

        this.queryRelations([req.params.id], true)
            .then(res);
    });
   ...
   *    }
    ...
    app.get('/relations', function (req, res) {
        ...
        // Query the relation by the type of tag it has.
            ...
            if(tagKeys.length === 0) {
                res.send(400, {error: 'Relations endpoint needs member or tags.'});
            }
        ...
        this.queryRelations(q).then(res);
    });
};
